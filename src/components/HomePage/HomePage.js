...
class HomePage extends React.Component {
    ...
    filterProducts = () => {
        ...
        value = this.props.products.filter((product) => product.name.toLowerCase().includes(this.state.filter.name.toLowerCase())
        )
        value = value.filter((product) => { return product.ibu >= this.state.filter.ibu[0] && product.ibu <= this.state.filter.ibu[1] }
        )
        value = value.filter((product) => { return product.abv >= this.state.filter.abv[0] && product.abv <= this.state.filter.abv[1] }
        )
                ...
                <TopSection >
                    ...
                    <SearchSection >
                        ...
                        <SearchGrid >
                            ...
                            <SearchInput value={this.state.filter.name} onChange={(ev) => { this.FilterAction("name", ev.target.value) }} />
                            <SubSearchTitle >IBU</SubSearchTitle>
                            <WrapperSlider>
                                <Box sx={{ width: "90%" }}>
                                    <Slider
                                        value={this.state.filter.ibu}
                                        onChange={(ev, newValue) => { this.FilterAction("ibu", newValue) }}
                            ...
                            <WrapperSlider>
                                <Box sx={{ width: "90%" }}>
                                    <Slider
                                        ...
                                        onChange={(ev, newValue) => { this.FilterAction("abv", newValue) }}
                                        valueLabelDisplay="auto"
                                        max={12}
                                    />
                                </Box>
